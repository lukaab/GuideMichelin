...
export default function DecouvrirScreen() {
  ...
  const featured = all.filter((r) => r.category !== 'Bib Gourmand');
  const discover = all.filter((r) => !user?.visitedRestaurants.includes(r.id) && r.category === 'Bib Gourmand');
    ...
    return (
      <ScrollView style={styles.desktopContainer} contentContainerStyle={styles.desktopContent}>
        <View style={styles.desktopHeader}>
          ...
          <TouchableOpacity style={styles.desktopSearchBar} onPress={() => router.push('/(tabs)/search')}>
            <Ionicons name="search-outline" size={18} color="#9B9B9B" />
            <Text style={styles.desktopSearchText}>Où voulez-vous dîner ?</Text>
          </TouchableOpacity>
        </View>
        <Text style={styles.sectionTitle}>À ne pas manquer</Text>
        <View style={styles.desktopGrid}>
          {featured.map((r) => (
            <RestaurantCardSmall key={r.id} restaurant={r} width={220}
              favorited={favorites.includes(r.id)} onFavorite={() => toggleFav(r.id)}
        ...
        <DetailModal restaurant={selected} user={user} onClose={() => setSelected(null)} onCheckin={handleCheckin} />
  ...
  return (
    <View style={styles.container}>
      ...
      {/* Search bar — blanc avec ombre (Mobile 1) */}
      <View style={styles.searchWrap}>
        <TouchableOpacity style={styles.searchBar} onPress={() => router.push('/(tabs)/search')} activeOpacity={0.85}>
          <Ionicons name="search-outline" size={17} color="#9B9B9B" style={{ marginRight: 8 }} />
          <Text style={styles.searchText}>Où voulez-vous dîner ?</Text>
        </TouchableOpacity>
      </View>
      ...
      <ScrollView style={styles.scroll} showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 32 }}>
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.rowContent}>
          {featured.map((r) => (
            <RestaurantCardSmall key={r.id} restaurant={r}
              favorited={favorites.includes(r.id)} onFavorite={() => toggleFav(r.id)}
              onPress={() => setSelected(r)} />
          ))}
        </ScrollView>
...
function DetailModal({ restaurant, user, onClose, onCheckin }: {
  restaurant: Restaurant | null; user: User | null;
  onClose: () => void; onCheckin: (r: Restaurant) => void;
}) {
  return (
...
const styles = StyleSheet.create({
  ...
  desktopSearchBar: {
    ...
    borderWidth: 1, borderColor: '#E5E5E5', gap: 10, maxWidth: 500,
  ...
  modalSheet: {
    ...
    padding: 16, paddingTop: 8, maxHeight: '85%',
  },
  modalHandle: { width: 36, height: 4, backgroundColor: '#D1D5DB', borderRadius: 2, alignSelf: 'center', marginBottom: 12 },
  toast: {
    position: 'absolute', bottom: 100, alignSelf: 'center',
    backgroundColor: '#1A1A1A', paddingHorizontal: 20, paddingVertical: 12, borderRadius: 24,
  },
